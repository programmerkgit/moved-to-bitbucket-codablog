...
export interface ApiFindOptions<T> extends LoggingOptions, SearchPathOptions {
    /**
     *  Custom Options
     */
    scopes?: string[];
    customScope?: 'full' | string;
    /**
     ...
     * two elements - the first is the title of the attribute in the DB (or some kind of expression such as
     * `Sequelize.literal`, `Sequelize.fn` and so on), and the second is the title you want the attribute to
     ...
     * `order: [['title', 'DESC']]`. In this way the column will be escaped, but the direction will not.
     */
     ...
     *
     * See [sequelize docs](https://github.com/sequelize/sequelize/blob/master/docs/transactions.md#after-commit-hook)
     * for an example of boolean usage.
     */
    lock?: boolean | string | {
        level: string;
        of: typeof Model;
